...
describe('rental routes', () => {
  ...
  beforeAll( async () => {
    ...
    owner = await mockRequest.post('/user').set('Authorization',`Bearer ${token}`).send(ownerData);
    borrower = await mockRequest.post('/user').set('Authorization',`Bearer ${token}`).send(borrowerData);
    ...
    item1 = await mockRequest.post('/item').set('Authorization',`Bearer ${token}`).send(item1data);
    item2 = await mockRequest.post('/item').set('Authorization',`Bearer ${token}`).send(item2data);
  ...
  it('can create new rental requests', async() => {
    ...
    rental1 = await mockRequest.post('/rentaldoc').set('Authorization',`Bearer ${token}`).send(rental1data);
  ...
  it('can update a rental request incrementaly', async() => {
    ...
    let updatedRental = await mockRequest.put(`/rentaldoc/${rental1.body._id}`).set('Authorization',`Bearer ${token}`);
  ...
  it('will get back all rental docs in the system', async () =>{
    rental2 = await mockRequest.post('/rentaldoc').set('Authorization',`Bearer ${token}`).send(rental2data);
    let rentalList = await mockRequest.get('/rentaldoc').set('Authorization',`Bearer ${token}`);
  ...
  it('will get back just the 1 desired rental doc', async () => {
    let rental2copy = await mockRequest.get(`/rentaldoc/${rental2.body._id}`).set('Authorization',`Bearer ${token}`);
  ...
  it('will only deactivate a finalized rental', async () => {
    ...
    let rental2copy = await mockRequest.delete(`/rentaldoc/${rental2.body._id}`).set('Authorization',`Bearer ${token}`);
    expect(rental2copy.status).toEqual(200);

    // at stage 2, can't be closed
    let rental1copyDelete = await mockRequest.delete(`/rentaldoc/${rental1.body._id}`).set('Authorization',`Bearer ${token}`);
    expect(rental1copyDelete.status).toEqual(406);

    // increment process
    await mockRequest.put(`/rentaldoc/${rental1.body._id}`).set('Authorization',`Bearer ${token}`);

    // at stage 3, can't be closed
    let rental1copy2delete = await mockRequest.delete(`/rentaldoc/${rental1.body._id}`).set('Authorization',`Bearer ${token}`);
    expect(rental1copy2delete.status).toEqual(406);

    // increment process
    await mockRequest.put(`/rentaldoc/${rental1.body._id}`).set('Authorization',`Bearer ${token}`);
    //at stage 4, can be closed
    let rental2copy3del = await mockRequest.delete(`/rentaldoc/${rental1.body._id}`).set('Authorization',`Bearer ${token}`);
