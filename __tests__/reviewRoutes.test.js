...
describe('review routes', () => {
  ...
  beforeAll( async () => {
    ...
    reviewWriter = await mockRequest.post('/user').send(testUser1);
    reviewSubject = await mockRequest.post('/user').send(testUser2);
  ...
  it('can create a user review', async () => {
    ...
    let r1 = await mockRequest.post('/review').send(review1);
  ...
  it('can create an item review', async () => {
    ...
    let review = await mockRequest.post('/review').send(review1);
    aReview = review;
  ...
  it('can get a review by it _id', async () => {
    let theAnswer = await mockRequest.get(`/review/${aReview.body._id}`);
    expect(theAnswer.body[0]._id).toEqual(aReview.body._id);
  });

  it('can get a review by the subject id', async () => {
    let itemReview = await mockRequest.get(`/review/${builtItem._id}/item`);
    let userReview = await mockRequest.get(`/review/${reviewSubject.body._id}/user`);
